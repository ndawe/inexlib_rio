...
namespace inlib {
    ...
    inline void unique(std::vector<T>& a_vec)
    {
        ...
        for (it = a_vec.begin(); it != a_vec.end(); ++it) {
            it2 = it;
            it2++; //INLIB_STL : it2=it+1 does not compile.
    ...
    template <class T>
    inline bool mn_mx(const std::vector<T>& a_vec, T& a_mn, T& a_mx)
    {
        if (a_vec.empty()) {
            a_mn = T();
            a_mx = T();
        ...
        for (it_t it = a_vec.begin(); it != a_vec.end(); ++it) {
            a_mn = *it < a_mn ? *it : a_mn;
            a_mx = *it > a_mx ? *it : a_mx;
        }
