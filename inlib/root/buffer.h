...
#ifndef inlib_root_buffer
#define inlib_root_buffer
...
// in INLIB_STL, we don't have (yet) a performant map.

#ifdef inlib_stl_vector
    #define INLIB_RROOT_OBJ_MAP_HASH_TABLE
    //#define INLIB_RROOT_OBJ_MAP_OMAP
#else
    #define INLIB_RROOT_OBJ_MAP_STD_MAP
#endif

#ifdef INLIB_RROOT_OBJ_MAP_STD_MAP
    #include <map>
#endif
...
namespace inlib {
    namespace root {
        ...
        class buffer : public buf {
            ...
            {
                static const std::string s_v("inlib::root::buffer");
                return s_v;
            }
        ...
        protected:
            buffer(const buffer& a_from)
                : buf(a_from.m_out, a_from.m_byte_swap, 0, m_pos)
        ...
        public:
            ...
            {
                return expand(mx<uint32>(2 * m_size, a_new_size));   //CERN-ROOT logic.
            }
            ...
            bool displace_mapped(unsigned int a_num)
            {
                ...
                {
                    inlib_vforcit(offset_id, m_cls_mapped, it) {
                        ...
                        if (!write(uint32(clIdx | kClassMask()))) {
                            m_pos = opos;
                            return false;
                        }
                    }
                ...
                //      << std::endl;
                {
                    inlib_vforcit(offset_id, m_obj_mapped, it) {
        ...
        protected:
            ...
            bool set_byte_count_obj(uint32 a_pos)
            {
                ...
                if (!m_wb.write(uint32(cnt | kByteCountMask()))) {
                    m_pos = opos;
                    return false;
                }
